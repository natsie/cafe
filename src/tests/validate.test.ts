...
import { assert, check, isNonNullable, nonNullable } from "../validate.ts";
import type { Assert } from "../types/validate.d.ts";
...
describe("Validators", () => {
  ...
  describe("assert", () => {
    const testAssert = (
      name: keyof Omit<Assert, "number" | "regexp">,
      validValue: any,
      invalidValue: any,
      message: string,
    ) => {
      describe(name, () => {
        it("should not throw for valid values", () => {
          expect(() => (assert[name] as any)(validValue)).not.toThrow();
        });
        it("should throw TypeError for invalid values", () => {
          expect(() => (assert[name] as any)(invalidValue)).toThrow(TypeError);
          expect(() => (assert[name] as any)(invalidValue)).toThrow(message);
        });
        it("should throw a custom message", () => {
          expect(() => (assert[name] as any)(invalidValue, "custom")).toThrow("custom");
        });
      });
    };
    ...
    testAssert(
      ...
      new Uint8Array(8),
      new ArrayBuffer(8),
      "The provided value is not an arraybuffer view",
    );
      ...
      describe(".positive", () => {
        ...
        it("should throw for zero or negative numbers", () => {
          expect(() => assert.number.positive(0)).toThrow(
            "The provided value is not a positive number",
          );
          expect(() => assert.number.positive(-1)).toThrow(
            "The provided value is not a positive number",
          );
        });
      });
      ...
      describe(".negative", () => {
        ...
        it("should throw for zero or positive numbers", () => {
          expect(() => assert.number.negative(0)).toThrow(
            "The provided value is not a negative number",
          );
          expect(() => assert.number.negative(1)).toThrow(
            "The provided value is not a negative number",
          );
        });
      });
      ...
      describe(".finite", () => {
        ...
        it("should throw for Infinity", () => {
          expect(() => assert.number.finite(Infinity)).toThrow(
            "The provided value is not a finite number",
          );
        });
      });
      ...
      describe(".safeInteger", () => {
        ...
        it("should throw for an unsafe integer", () => {
          expect(() => assert.number.safeInteger(Number.MAX_SAFE_INTEGER + 1)).toThrow(
            "The provided value is not a safe integer",
          );
        });
      });
    });
