...
import { Logger, createConsoleWriter, createLogFileWriter } from "../logger.ts";
import type { Log, LogWriter } from "../types/logger.d.ts";
...
describe("Logger", () => {
  ...
  it("should call the writer of an active interface", () => {
    const logger = new Logger();
    const writer = mock((l: Logger, log: Log) => {});
    logger.addInterface("test", writer);
    logger.log("test message");
    expect(writer).toHaveBeenCalledTimes(1);
    expect(writer).toHaveBeenCalledWith(
      logger,
      expect.objectContaining({ type: "log", data: ["test message"] }),
    );
  });
