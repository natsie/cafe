...
import type { Config } from "./types/cafe.d.ts";
...
for (const arg of argv.slice(2)) {
  ...
  if (option.startsWith("--")) {
    if (currentOption) {
      throw new Error(
        `Expected configuration option value after --${currentOption}, received ${option}`,
      );
    }

    option = option.slice(2);
    if (booleanOptions.has(option)) (config[option as keyof typeof config] as any) = true;
    else currentOption = option;
    continue;
  }
  ...
  switch (currentOption) {
    ...
    case "exclude": {
      if (option.startsWith("regexp:")) {
        config.menu[currentOption].push(
          new RegExp(
            option.slice(8, option.lastIndexOf("/")),
