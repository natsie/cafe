...
import { createReadStream, createDeferred, obj, str, numberToBytes, bytesToNumber, readJSONFromFile } from "../utils.js";
describe("Utilities", () => {
    ...
    describe("obj", () => {
        ...
        describe("pick", () => {
            ...
            it("should handle non-existent keys by assigning undefined", () => {
                const result = obj.pick(sourceObj, ["a", "e"]);
                expect(result).toEqual({ a: 1, e: undefined });
                expect("e" in result).toBe(true);
            });
            it("should return an empty object when picking from an empty object", () => {
                const result = obj.pick({}, ["a"]);
